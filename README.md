...
恋愛ゲーム風ブラウザノベルゲームの、拡張しやすいプロトタイプです。主人公は画面に登場する固定キャラクターではなく、遊んでいるプレイヤー本人です。
...
直前の選択肢は自動的にチェックポイントとして保存され、「直前の選択肢からやり直す」で戻れます。`id` は各BAD ENDで必ず一意にしてください。
...
- 左上のホームボタンからタイトルに戻れます。進行は各ノード表示時に自動保存されます。
